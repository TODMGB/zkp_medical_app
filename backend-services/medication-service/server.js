...
// 5. 认证中间件，从 API Gateway 传递的请求头中提取用户信息
...
// 6. 处理未匹配到的路由 (404 Not Found)
app.use(notFoundHandler);

// 7. 统一的错误处理器，捕获所有从路由和中间件传递过来的错误
...
async function startServer() {
  try {
    ...
    // 5. 启动MQ消费者（可选，如果服务不需要消费MQ消息可以注释掉）
    await startConsumers(wss);
    console.log('✅ MQ消费者已启动');

    // 6. 启动HTTP服务器监听指定端口（HTTP和WebSocket共用同一端口）

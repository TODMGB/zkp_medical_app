...
async function startServer() {
  try {
    ...
    console.log('🔧 正在初始化 Secure Exchange Service...');
    console.log('='.repeat(60));

    // 1. ✅ 初始化MQ连接（作为Producer，确保能发送通知）
    const { getChannel } = require('./src/mq/client');
    try {
      await getChannel();
      console.log('✅ RabbitMQ Producer已连接');
    } catch (mqError) {
    ...
    // 3. 创建WebSocket服务器（使用同一个HTTP server）
    ...
    console.log('✅ gRPC服务器已启动');

    // 8. 启动HTTP服务器监听指定端口（HTTP和WebSocket共用同一端口）
    server.listen(config.PORT, () => {
      console.log('='.repeat(60));
      console.log('🚀 Secure Exchange Service 启动成功！');
      console.log('='.repeat(60));
      console.log(`📡 HTTP API:    http://localhost:${config.PORT}`);
      console.log(`🔌 WebSocket:   ws://localhost:${config.PORT}${config.websocket.path}`);
      console.log(`🔐 gRPC:        localhost:${config.GRPC_PORT || 50052}`);
